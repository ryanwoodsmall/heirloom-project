...
/*	from OpenSolaris "malloc.c	1.32	05/06/08 SMI"	*/

/*
 * Portions Copyright (c) 2005 Gunnar Ritter, Freiburg i. Br., Germany
 *
 * Sccsid @(#)malloc.c	1.3 (gritter) 8/26/05
 */
...
#define	NPS (WORDSIZE*8)
		...
		if ((List[i] = malloc((size + WORDSIZE) * NPS)) == 0)
			return (0);
...
#endif	/* MAP_ANON */
	flags |= MAP_PRIVATE;
	return mmap(NULL, len, PROT_READ|PROT_WRITE, flags, fd, 0);
}
#endif	/* SEGMENTED */
