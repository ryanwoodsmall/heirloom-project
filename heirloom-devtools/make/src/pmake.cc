...
/*	from OpenSolaris "pmake.cc	1.9	06/12/12"	*/
				...
				if (c != '\n')  {
					...
					while ((*ms != '\n') && (*ms)) {
						if (*ms == '#') {
							...
							warning("unknown option for host %s", mbs_buffer);
							SKIPTOEND(ms);
							break;
						}
					}
...
get_max(wchar_t **ms_address, wchar_t *hostname)
{
	...
	if ((!*ms) || (*ms == '\n') || (*ms != '=')) {
		SKIPTOEND(ms);
		warning("expected `=' after max, ignoring rest of line for host %s",
			mbs_buffer);
		*ms_address = ms;
		return((int) limit);
	} else {
		...
		if ((*ms != '\n') && (*ms != '\0')) {
			...
			if (limit < 1) {
				...
				warning("max value cannot be less than or equal to zero for host %s", mbs_buffer);
			}
		} else {
			/* No "max" value after "max=". */
			warning("no max value specified for host %s", mbs_buffer);
		}
		*ms_address = ms;
		return(limit);
	}
